...
(async () => {
  ...
  try {
    ...
    // ---------- 5b. 导出必须**导满**（钉住一个真实发生过的静默截断）----------
    //
    // 背景：单表导出原先调 `page_rows(..., usize::MAX)`，而分页有硬上限
    // （`MAX_PAGE_LIMIT = 500`）—— **超过 500 行的表只会导出前 500 行，且不报错**。
    // 这里用现成的 **1200 行**表验：两个出口导出的行数都必须是 1200。
    //
    // 为什么这条放在端到端而不是单测：单测只能证明"函数返回 1200 行"，
    // 而这里证明的是**从界面按钮那条 IPC 走下来**也导满 —— 同一类错误
    // 完全可能只在接线那一层（传参数的地方）出现。
    const xr = await ipc("export.table", { name: TABLE });
    step(
      "「导出当前表格」导满 1200 行（曾被静默截到 500）",
      Number(xr && xr.count) === 1200,
      "导出了 " + (xr && xr.count) + " 行"
    );
    const hr = await ipc("report.exportTable", { name: TABLE });
    step(
      "「打印当前表格」也导满 1200 行",
      Number(hr && hr.count) === 1200,
      "导出了 " + (hr && hr.count) + " 行"
    );
    step(
      "打印视图是自包含的 .html（不引任何外部资源）",
      !!(hr && /\.html$/.test(hr.path || "")),
      hr ? hr.path : "无返回"
    );
