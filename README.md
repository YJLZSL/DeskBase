...
**本地优先 · 隐私优先的办公工具箱，内置人人可上手的数据表引擎（多表 + 关联 + 协同更新，无 SQL）**
...
| **数据库** | 数据表引擎（自研单文件存储，无 SQL）：可视化建表（12 类字段类型）、数据网格（虚拟滚动 / 排序 / 筛选 / 单元格编辑 / 批量删行）、命名视图（筛选 / 排序 / 分组 / 隐藏列，替代查询语句）、关联字段 / 共通字段 / 同步规则（可配置、可追溯）。金额按「分」精确存储 |
...
| 截图历史 · 滚动长图 | 单帧 / 站内长图 / **滚动捕获（能截外部应用）**/ 历史网格，可删 |
| **AI 对话** | 能问、能答，**默认只发结构不发数据行**；默认走本机模型，数据不出本机。带常驻隐私面板与发送记录 |
| **表格出口** | 单表导出 `.xlsx` · **打印视图**（A4 横向、表头每页重复，浏览器里 Ctrl+P 即可存 PDF）· 全库导出 CSV/Markdown |
| **安装版** | 便携版点「安装到本机」即可：开始菜单出现入口、可在「应用和功能」里正常卸载，**桌面快捷方式可选** |
...
| **AI 自动改表 / 自然语言转 SQL** | [ADR-0017](docs/adr/0017-ai-assistance-privacy-boundary.md) 要求"AI 给的东西不许自动执行"。AI 对话已经能用，但**它只给建议，一个字都不落库** |
| **数据库业务表** | 库表引擎已上线。物流 / 财务 / 工厂收发那几张业务表还没建 |
| **多语言** | 目前只有中文界面。i18n 是独立议题，排在 v1.0 之后 |
| **加密** | 排在 v1.0 之后。字段级加密会牺牲查询与排序能力，不推荐；要做就走库级加密 |
| **看板 / 日历 / 待办 / 文件管理 / OCR / 剪贴板历史** | 评估见 `local-docs/reference/32-tech-feasibility.md`：OCR 有条件做（体积代价仅 +3.5~11 KB），剪贴板历史**明确不做常驻监听** |

完整清单在 [CHANGELOG](CHANGELOG.md) 的「已知未完成」一节。
工程状态：`cargo test` **全绿**、编译零警告、门禁全绿（动效合规、对比度 11 主题、接线五项、体积守卫 10 MB）。
界面烟测真实点击、界面自动走查 4 档宽度 × 4 页布局 0 处、导入端到端、崩溃恢复套件、导入压力测试。
**联网测试**（更新通道 + 安装）标了 `#[ignore]`，需要时用 `cargo test -- --ignored` 跑。
...
DeskBase（桌库）是一个**本地优先（local-first）**的桌面办公增强层。
定位是 **Office 三件套的扩展与升级 + 办公实用功能集**（[ADR-0023](docs/adr/0023-office-suite-extension.md)）：
**不替代 Office，补它嫌小不做、做了又别扭的那块。**

它想解决的问题很朴素：

> 普通人每天在电脑上产生大量「半结构化」信息——待办、清单、截图、通讯录、记账、库存、客户名单、读书笔记——把它们记在备忘录里会乱，放进 Excel 会卡，学真正的数据库管理工具又太陡。

DeskBase 的做法是：**把办公里反复要做的杂事收进一个 exe**。

打开就能记（笔记）、截（长图）、算（表格）、转（格式转换）、查（台账的筛选与视图）；
等你哪天想认真一点，关联字段、共通字段、同步规则、命名视图就在旁边，一步之遥，不逼你学。
**没有查询语句，也没有查询输入框** —— 想怎么筛，存成一个有名字的视图就行。
...
### 数据表引擎（无 SQL）
可视化建表、字段编辑、关联字段、共通字段、同步规则（可配置方向、可配置冲突策略、总开关、可追溯）、查找与汇总字段、命名视图（筛选 / 排序 / 分组 / 隐藏列，**无查询输入框**）、索引。支持 CSV / JSON / Excel / SQLite 导入导出（导出侧允许 SQLite，便于带走），数据校验、权限控制、审计日志、版本历史、API 生成。

对新手：**图形化建表 + 命名视图**；对高级用户：**字段类型、关联、共通字段、同步规则全部可视化配置**，不阉割，但界面不提供任何查询语言。
...
### 本地 AI
自然语言查询（→ 视图筛选条件）、数据摘要、分类、翻译、OCR、语音转写、智能搜索、报表解读。**模型、向量索引、对话记录、临时数据全部留在本地**，禁止默认调用云端 API。若用户主动选择外部 AI 服务，需显式授权、逐次确认、可脱敏、可审计、可完全关闭。
...
| | 状态 |
|---|---|
| 便携版（解压即用） | ✅ 已发布（默认形态，**不写一个字节的注册表**） |
| 安装到本机 | ✅ 设置页一键安装（**用户级**，不需要管理员，不碰 `HKLM`） |
| Linux / macOS | ❌ 暂不支持（架构本身跨平台，只做 Windows 是集中资源的选择） |
| 代码签名 | ❌ 没有证书，SmartScreen 会拦 |

> **便携版与安装版有什么区别**：便携版解压即用、零注册表；安装版把程序放到
> `%LOCALAPPDATA%\Programs\DeskBase`，并在 Windows 的「应用和功能」里留一条**可正常卸载**的记录。
> 两者是**同一个 exe** —— 装不装由你点不点那个按钮决定，程序不会擅自写注册表。
> 卸载会清掉程序文件与注册表项，**但保留你的数据目录**（卸载程序不该替用户决定他的报表要不要留）。
>
> **SmartScreen 会拦**：没有代码签名证书，所以 Windows 可能提示"未知发布者"。
> 校验用的 SHA-256 与文件清单来自同一个 Release —— 账号一旦被攻破，这条链会一起失效。
> 想更稳妥就自己到 Releases 手动下载、核对 `.sha256` 再覆盖。
>
> `0.x` 期间不承诺向后兼容，**请勿用于唯一的生产数据**。
> 已知未完成的部分逐条列在 [CHANGELOG](CHANGELOG.md) 里。
...
| [docs/06-data-table-engine.md](docs/06-data-table-engine.md) | 数据表引擎设计（无 SQL） |
