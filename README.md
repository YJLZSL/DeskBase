...
工程状态：`cargo test` **263** 个用例全过、编译零警告、门禁全绿（动效合规、对比度 11 主题、接线五项、体积守卫 10 MB）。
界面烟测 **91** 步真实点击、界面自动走查 4 档宽度 × 4 页布局 0 处、导入端到端 27 步、
崩溃恢复套件 16/16、导入压力测试 22/22 × 2 档。
**联网测试**（更新通道 3 项 + 安装 1 项）标了 `#[ignore]`，需要时用 `cargo test -- --ignored` 跑。
...
**当前版本：v1.4.0**（第一个正式大版本）。便携版与安装版是**同一份产物**，见 [Releases](https://github.com/YJLZSL/DeskBase/releases)。
