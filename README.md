...
在此之上提供 **9 个内置主题**（宣纸 / 青瓷 / 竹青 / 朱墨 / 夜墨 / 松烟 / 玄铁 / 高对比亮 / 高对比暗）与「跟随系统」——跟随系统时随操作系统的亮暗设置自动切换（事件驱动，不轮询）。主题切换用整屏交叉淡入完成，不闪白。

**应用图标**是宣纸底上的一枚朱砂「库」印：印章即落款与归属，正对「数据属于你自己」这条承诺。

**动效只有四个档位**——`关` / `精简` / `标准` / `丰富`，随时可切。原则是**服务于理解，而不是吸引注意力**：拖拽 1:1 跟手、操作 100 ms 内有反馈、只用 `transform` 与 `opacity`、不做任何循环播放的装饰动画。档位调到「关」是把时长压到 1 毫秒，而不是把动画藏起来——元素仍然停在正确的位置，不会有东西卡在半路。

完整规范见 [docs/18](docs/18-visual-and-motion-design.md)。

**字体三槽位可分别配置**（标题 / 正文 / 代码），内置 5 个预设，并支持导入 `.ttf` `.otf` `.woff2` `.woff` 自定义字体。

标题槽位内嵌 **得意黑 Smiley Sans**（SIL OFL 1.1，WOFF2 仅 **1.10 MB**，随程序分发、不联网下载）；正文与代码走系统字体栈——正文用字量最大，内嵌中文字库得不偿失。不想用内置字体可在设置里一键切回系统宋体系。
