...
[![Status](https://img.shields.io/badge/status-beta-blue.svg)](#-项目状态beta数据核心已上线)
...
| **数据库** | 可视化建表（9 种字段类型）、数据网格（虚拟滚动 / 排序 / 筛选 / 单元格编辑 / 批量删行）、SQL 编辑器（语法高亮 / 危险操作双重确认 / 执行历史）。金额按「分」精确存储 |
...
| **表结构编辑** | 改表名 / 加列 / 改注释的后端与测试已就绪，界面（建表向导二期）还没做 |
| **数据库业务表** | 库表引擎已上线。物流 / 财务 / 工厂收发那几张业务表还没建 |
| **AI 功能** | 一个都没接。选型与隐私方案已定稿（全本地优先，云端排最后） |

完整清单在 [CHANGELOG](CHANGELOG.md) 的「已知未完成」一节。
工程状态：`cargo test` **151** 个用例全过、编译零警告、门禁全绿（动效合规 7 份样式表、对比度 11 主题）。
...
**只有便携版**，见 [Releases](https://github.com/YJLZSL/DeskBase/releases)（最新为 `v0.2.0-beta.3`，**预发布**）。
