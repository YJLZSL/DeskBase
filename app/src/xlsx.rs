...
///
/// 三条拒绝理由，每一条都对应一类真实的数据损坏：
///
/// 1. **15 位以上的纯数字** —— 那多半是订单号/身份证/银行卡。
///    IEEE-754 双精度只有 15–17 位有效数字，超了就当数值写进 Excel，
///    末几位会被**永久**改成 0，事后把单元格格式改成文本也救不回来。
/// 2. **前导零**（`007`、`0123`）—— 商品编码、工号、行政区划码大量长这样。
///    当数值写就是 `7` / `123`，前导零没了，而且**没人会立刻发现**，
///    等对账时才知道编号全对不上。注意 `0.5` 是合法的数，`0` 也是；
///    被拒的是"0 后面还跟数字"这种（`0.` 后面是小数点，不在其列）。
/// 3. **科学计数法写法**（含 `e`/`E`）—— 用户给的原文如果是 `1e5`，
///    那是文本，不是要我们替他解释成 100000。
///
/// 剩下的才当数值写，好处是在 Excel 里能直接求和、排序。
...
mod tests {
    ...
    fn 前导零的编号必须当文本写() {
        ...
        // （商品编码、工号、行政区划码、班级号）。
        for s in ["007", "0123", "001", "00"] {
            assert!(!is_plain_amount(s), "{s} 有前导零，是编码不是数量，必须当文本");
        }
        // 但不能矫枉过正：这些是真的数值
        for s in ["0", "0.5", "-0.5", "7", "12.34", "100", "-3"] {
            assert!(is_plain_amount(s), "{s} 是普通数值，应当能当数值写");
        }
        // 小数点在第二位的情况：0.5 合法、007 非法 —— 区别就在 b[1] 是不是数字
        assert!(is_plain_amount("0.5"));
        assert!(!is_plain_amount("00.5"));
    }
