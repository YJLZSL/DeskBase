...
fn dispatch_sync(state: &AppState, req: Request) -> String {
    ...
    match req.cmd.as_str() {
        ...
        // ---------- 安装版 ----------
        // 便携版零注册表；只有用户主动点「安装到本机」才写，且全落在 HKCU。
        "app.installState" => {
            ok(id, serde_json::to_value(installer::state(&state.data_dir)).unwrap_or_default())
        }
        ...
        "app.uninstall" => {
            // 二次确认放在界面上做（这里不再拦一道，否则自动化测试没法验）
            match installer::uninstall() {
                Ok(msg) => {
                    log_line(&state.data_dir, "已卸载（用户数据目录保留）");
                    ok(id, serde_json::json!({ "message": msg }))
                }
                Err(e) => err(id, e),
            }
        }
